...
--    PERF: the mode was originally a correlated scalar subquery keyed on authority_eik. An
--    authority_eik index on raw_contracts/raw_tenders does NOT make SQLite push the correlated
--    predicate into the UNION ALL derived table `v` — EXPLAIN QUERY PLAN shows a full re-SCAN of
--    both staging tables per outer row (O(groups×rows); ~46s against a 4874-authority/12-row-each
--    fixture). Precompute the mode once — a single GROUP BY + ROW_NUMBER() window pass over the
--    union, O(n log n) — into authority_canonical_name, then join it in below (indexed point
--    lookup per group; same fixture drops to well under 100ms total). A plain (non-TEMP) scratch
--    table, matching refresh-slice.sql's refresh_touched_* convention — D1 batches don't guarantee
