...
-- Canonical name is the MODE per ЕИК (not MIN), multi-ЕИК joint-procurement lists are excluded —
-- kept identical to normalize-raw.sql step 1 so the incremental path and the full rebuild agree
-- (#194 / #196; parity test in packages/db/src/refresh-slice.test.ts). PERF: the mode used to be a
-- correlated scalar subquery, which EXPLAIN QUERY PLAN shows SQLite does NOT index-seek — it
-- re-SCANs raw_contracts/raw_tenders per outer row regardless of the authority_eik index
-- (O(groups×rows)). Precompute the mode once via GROUP BY + ROW_NUMBER(), then join it in — same
-- fix as normalize-raw.sql step 1. Plain (non-TEMP) scratch table, matching refresh_touched_*
...
FROM (
  SELECT
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY
        cnt DESC,                                                       -- mode: most-frequent label wins
        CASE WHEN authority_name GLOB '*[a-zа-я]*' THEN 0 ELSE 1 END,   -- prefer mixed-case over ALL-CAPS
        LENGTH(authority_name) DESC,                                    -- then the more descriptive (longer) label
        authority_name                                                  -- final deterministic tiebreak
    ) AS rn
  FROM (
...
WHERE (s.source LIKE 'eop:%' OR s.source LIKE 'ocds:%') AND s.authority_eik IS NOT NULL
  AND s.authority_eik NOT LIKE '%;%'   -- exclude multi-ЕИК joint-procurement lists (#196)
    ...
    FROM (
      ...
      FROM (
        SELECT
          contractor_name,
          eik_clean,
          -- Same Bulgarian ЕИК checksum as the bidder INSERT above and normalize-raw.sql (#195),
          -- so the touched-bidder key matches the actual bidders.id keying.
          CASE
            WHEN eik_clean IS NULL OR eik_clean GLOB '*[^0-9]*' OR LENGTH(eik_clean) NOT IN (9, 13) THEN 0
            WHEN (
              CASE
