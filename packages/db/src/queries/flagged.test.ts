...
describe('getFlaggedValue', () => {
  it('de-duplicates the total (a contract with two signals counts once)', async () => {
    ...
    // c1..c5 flagged (c6 clean). c4 is value_suspect → NULL amount_eur → counted, €0.
    expect(f.contracts).toBe(5);
    expect(f.totalEur).toBe(6500); // 1000 + 2000 + 1500 + 0 + 2000
  });

  it('reports overlapping by-type slices (their sum exceeds the de-duplicated total)', async () => {
    const f = await getFlaggedValue(realDb());
    expect(byType(f, 'no_competition')).toMatchObject({ eur: 3000, contracts: 2 }); // c1 + c5
  ...
  it('partitions by sector so the slices sum to the total', async () => {
    const f = await getFlaggedValue(realDb());
    ...
    expect(f.bySector.reduce((n, s) => n + s.eur, 0)).toBe(f.totalEur);
  });

  it('partitions by authority type so the slices sum to the total', async () => {
    const f = await getFlaggedValue(realDb());
    ...
    expect(f.byAuthorityType.reduce((n, a) => n + a.eur, 0)).toBe(f.totalEur);
  });
});
...
describe('/contracts flag filter', () => {
  ...
  it('flag=all matches every flagged contract', async () => {
    ...
    expect(r.total).toBe(5); // c1..c5, not the clean c6
  });
  ...
  it('multiple flag tokens are OR-combined (union)', async () => {
    ...
    expect(r.total).toBe(3); // c1, c5 (no_competition) ∪ c4 (anomalies)
  });
