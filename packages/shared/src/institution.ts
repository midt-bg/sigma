...
export function identityInstitution(name: unknown): string {
  ...
  return (
    t
      ...
      .replace(/,\s*(?:ГР|С)\.\s*[^,]+$/u, '')
      // „Община Тестовци", „ОбС Тестовци", „Общински съвет - Тестовци" and the listing's bare „Тестовци".
      .replace(
        /^(?:ОБЩИНА|ОБЩИНСКИ СЪВЕТ|ОБЩ\.?\s*СЪВЕТ|ОБС)(?=[\s,–—-])\s*(?:[-–—,]\s*)?(?:НА\s+)?(?:ОБЩИНА\s+)?(?=\S)/u,
        '',
      )
      .replace(/^(?:ГР|С)\.\s*/u, '')
      // „Област - Тестово" and „Областна администрация - Тестово" — an oblast stays apart from its town.
      .replace(
