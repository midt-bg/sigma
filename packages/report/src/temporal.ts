...
/** Split a well-formed `YYYY-MM-DD` into its numeric parts. Callers only ever pass strings already
 * shaped by this module (or validated by `isValidIsoDate`), so a malformed split degrading to `NaN`
 * (never a thrown/undefined index) is an acceptable internal invariant. Exported for `./iso-week`,
 * which shares the same Monday-anchored day arithmetic. */
...
/** Add `n` days to an ISO date, DST-immune (UTC-noon anchor). Exported for `./iso-week`. */
...
/** Weekday of an ISO date, Monday=0 .. Sunday=6. Exported for `./iso-week`. */
...
function detectExplicitCalendar(q: string, a: Anchor): ResolvedPeriod | null {
  ...
  for (const re of ISO_RANGE_PATTERNS) {
    ...
    const m1 = m?.[1];
    const m2 = m?.[2];
    if (m1 && m2 && isValidIsoDate(m1) && isValidIsoDate(m2)) {
      const lo = minIso(m1, m2);
      const hi = m1 === lo ? m2 : m1;
      return period(
        'explicit-range',
        `${lo}–${hi}`,
        `${lo} – ${hi}`,
  ...
  const day1 = day?.[1];
  if (day1 && isValidIsoDate(day1)) {
    return period('explicit-day', day1, day1, day1, addDaysIso(day1, 1), 'day', a);
  }
