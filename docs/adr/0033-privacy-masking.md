# ADR-0033 — Политика за поверителност на машинно-четим изход (Issue #173)
...
- **Виж също:** [ADR-0034](../adr/0034-centralized-x-robots-tag-worker.md) — инженерното решение за централизирано авторство на `X-Robots-Tag: noindex` в worker-а (рефакторът от PR #183 review) и [`docs/privacy-masking.md`](../privacy-masking.md) за оперативно ръководство.
...
HTML профилът на фирма вече прилагаше `noindex` мета таг за записи, които се разпознават като едноличен търговец или физическо лице (`isSingleNaturalPersonProfile` в [`apps/web/app/routes/company.tsx`](../../apps/web/app/routes/company.tsx) — *inline*, преди ADR-0033). Логиката имаше две слабости:
