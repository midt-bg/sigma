...
## `feat/weekly-digest` → `main` merge checklist (producer-side, out of this consumer PR)
Pre-existing on the umbrella branch (not introduced by #167B) — verify before the umbrella merge (#81 review, item 2):
- **`apps/etl/wrangler.toml` fail-dark posture:** `workers_dev = true` contradicts the line-7 comment (which asserts `workers_dev = false`, no public route), and `DIGEST_TRIGGER_ENABLED = "true"` — together they leave the on-demand digest trigger reachable, where the committed prod defaults are both `false`. Flip both back to `false` for prod.
- **Rotate `DIGEST_TRIGGER_TOKEN` on `sigma-etl-dev`** — its test value was a known short string.
