...
| `200` | **грешка по време на streaming: gateway 429** (глобалният BgGPT лимит в AI Gateway се задейства при model call, mid-pipeline) | четим текст в стрийма през `onError`: `Системата е натоварена, опитай пак след малко.` | в SSE стрийма | — |
...
| v3     | 2026-07-06 | Mid-stream gateway 429 получава отделно съобщение („Системата е натоварена, опитай пак след малко.") в SSE error chunk-а. |
| v4     | 2026-07-08 | Премахнати редовете за 405/403/415 и параграфът за First-party (CSRF/DoW) guard. Защитата по метод/произход/съдържание и bot/CSRF вече се налага на ниво маршрут и ръб — Turnstile edge gate (`app/lib/assistant/turnstile.ts`) + проверките в самите route handler-и (`routes/assistant.*.tsx`), а не в този контракт-документ. |
