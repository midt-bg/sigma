...
**Since September 2026 the register is not asked per ЕИК at all ([ADR-0041](../adr/0041-registry-layer-in-the-daily-etl.md)).** The daily ETL reads each procurement winner's partida from the register's API into the registry layer (`registry_deeds`, `registry_roles`), and the evidence ladder reads that layer — the roles with their entries, the legal form, the settlement of the seat. The portal measurements below describe the path this replaced.

**Phase-0 is CLOSED — measured 2026-08-04/05 (#279, ADR-0033).** Endpoint: `GET portal.registryagency.bg/CR/api/Deeds/{ЕИК}`, public JSON, no authentication. Facts worth recording because three of them contradict what the issue predicted:
- **The register rate-limits, and the block is short.** Re-measured 2026-08-19 ([ADR-0036](../adr/0036-tr-rate-limit-remeasured.md), which supersedes the Phase-0 reading below): the budget is **5 requests**, the block is scoped to the **IP** (a fresh session is refused too), there is no `Retry-After` and no quota header, and it **clears on its own in ~161 s**. The original Phase-0 note recorded ~50 cumulative requests and a *sustained* block, and concluded that a 429 should end the run; both halves of that have since been measured false. A 429 is therefore a **cooldown**: the crawler waits it out and re-requests the same ЕИК, and only a block outlasting three cooldowns ends the run. The pace floor is unchanged at 1 request / 3 s, sequential, over a **closed** candidate set the crawler cannot extend (it never follows a link out of a deed). §3.3 still governs — the threshold is not to be probed further.
- **„Not in the register" is an HTTP 200 with a ZERO-BYTE body**, not a 404 and not HTML. Verified on Община София (`000696327`): empty twice while a real company returned its full deed in the same window. Permanence therefore keys on the **status**, not the empty body — empty under 200 is an answer and may be cached; empty under 5xx is a failure and stays transient.
- **`fetch` does not work against this host.** The identical request returns 500/empty via undici and 200 with the full deed via `node:https`.
- Field values are HTML fragments inside the JSON envelope, and one field routinely holds several people separated by `<hr class='hr--report'/>`; erasure is marked structurally (`erasure-text-inline`), not textually. Matching must happen **inside one entity**, or one person's given name combines with another's surname.
- **ЕГН is absent** from every payload examined, and is never stored either way. Raw deeds are never written to disk at all on the decision path ([ADR-0037](../adr/0037-verdict-cache-crosses-the-run-boundary.md)) — the crawler decides beside the response and keeps only the verdict, which carries a kind, a role, an entry reference and booleans, and no name of anyone without public office. The 35-day deed retention still governs the `--eiks-file` path, where a deed is written; verdicts age on their own longer window.
...
| `company_owners` → `registry_roles` | one registered role | the registry layer ([ADR-0041](../adr/0041-registry-layer-in-the-daily-etl.md)) | `eik`, `field_ident`, `role`, the holder (the register's person identifier or the ЕИК), name, `share`, `entry_number`, `added_on`, `removed_on`. No ЕГН stored. |
...
- **Libel gate PASSED: 0 true normalizer over-merges** — proven on the **labelled** stratified sample (`company-name-key.test.ts`, ground-truth `companyId`, bar 0). That labelled test is the *sound* proof; the loader's self-comparison of the winner set cannot reproduce it (it is a structural false-zero — ADR-0027). Instead the loader reports **`ambiguous_name_keys`** as telemetry (a key → >1 valid ЕИК): 54 on the current corpus, every one a presentation-only collision (generic names / feed typos), all quarantined, **0 published**. The normalizer only ever folds presentation (case/space/quotes).
