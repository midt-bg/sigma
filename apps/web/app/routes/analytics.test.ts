...
describe('loader (/analytics)', () => {
  it('fans out five bounded reads and derives peak/YoY/opaque from real trend points', async () => {
    ...
    // Fewer than two complete years on record → an explicit no-data state, never a measured 0%.
...
describe('loader (/analytics) — YoY growth', () => {
  const month = (year: number, valueEur: number) =>
  ...
  it('reports the median year-over-year growth as a ratio once two complete years exist', async () => {
    q.getOverrunsHeadline.mockResolvedValue({ totalOverrunEur: 0, count: 0 });
    q.getFlowsHeadline.mockResolvedValue({ totalEur: 0 });
    q.getRegionHeadline.mockResolvedValue({ regions: 0 });
    q.getSpendingTrend.mockResolvedValue({ points: [...month(2022, 100), ...month(2023, 120)] });
    q.getOpaqueShareByYear.mockResolvedValue([]);

    const res = await loader({ context } as never);

    expect(res.trend.avgYoy).toBeCloseTo(0.2, 5); // 100 → 120 = +20%, a ratio (not the 1.2 multiplier)
  });
});
