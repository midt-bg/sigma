...
const stubDb = (contracts: unknown[] = [], groups: unknown[] = []) => {
  q.getSpendingTrend.mockResolvedValue(EMPTY_TREND);
...
describe('trends loader — param validation', () => {
  it('falls back to the safe default for every unrecognised or hostile param', async () => {
    stubDb();
    const data = await call('?angle=evil&step=x&sort=random&cpvSort=zzz&year=1999');
  ...
  it('accepts each documented value', async () => {
    stubDb();
    const data = await call('?angle=cross&step=m&sort=value&cpvSort=med&year=2024');
    expect(data).toMatchObject({
      angle: 'cross',
      step: 'm',
      sort: 'value',
    ...
    // The cross lens always shows the compact quarterly picker, whatever step was asked for.
    expect(q.getSpendingTrend).toHaveBeenCalledWith(
      DB,
  ...
  it('follows the step toggle to the chart granularity on the time lens', async () => {
    stubDb();
    await call('?step=y');
    expect(q.getSpendingTrend).toHaveBeenLastCalledWith(
      DB,
  ...
  it('sorts the selected cpv groups so a reordered URL shares one canonical selection', async () => {
    stubDb();
    const data = await call('?cpv=45233&cpv=33600');
    expect(data.cpvSel).toEqual(['33600', '45233']);
  });
});

describe('trends loader — median baselines', () => {
  it('asks for medians only for card groups and selected groups the top-N stats lack, once each', async () => {
    const known = {
      group: '11111',
      name: 'known',
      contracts: 5,
