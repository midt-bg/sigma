...
describe('/analytics route — render', () => {
  ...
  it('says there is not enough data — not a measured 0% — when the growth estimate is unavailable', async () => {
    const data = loaderData(null);
    (data.trend as { avgYoy: number | null }).avgYoy = null;
    await renderAnalytics(data);
    const trendCard = [...container.querySelectorAll('.az-card')].find((c) =>
      c.textContent?.includes('СРЕДЕН РЪСТ'),
    )!;
    const label = trendCard.querySelector('.metric-info-btn')!.getAttribute('aria-label') ?? '';
    expect(label).toContain('Типичният годишен ръст');
    expect(trendCard.textContent).toContain('Няма достатъчно пълни години данни');
    expect(trendCard.textContent).not.toContain('0%/год');
  });

  it('shows the estimated growth and points at the Тренд page when it is available', async () => {
    await renderAnalytics(loaderData(null));
    const trendCard = [...container.querySelectorAll('.az-card')].find((c) =>
      c.textContent?.includes('СРЕДЕН РЪСТ'),
    )!;
    expect(trendCard.textContent).toContain('+12%/год');
    expect(trendCard.textContent).toContain('Същата стойност като на страницата „Тренд"');
  });
});
