...
describe('/trends page render', () => {
  ...
  it('flags a truncated first page and describes the scope of the list', () => {
    const many = Array.from({ length: 24 }, (_, i) => ({ ...contract, id: `c${i}` }));
    render(
      '/trends?year=2024&cpv=45233',
      loaderData({ contracts: many, contractsTruncated: true, year: '2024', cpvSel: ['45233'] }),
    );
  ...
  it('does not claim a cut when exactly one full page matched', () => {
    const many = Array.from({ length: 24 }, (_, i) => ({ ...contract, id: `c${i}` }));
    render('/trends', loaderData({ contracts: many, contractsTruncated: false }));
    expect(container.textContent).not.toContain('показани първите');
  });
