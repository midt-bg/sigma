...
describe('/trends page render', () => {
  ...
  it('says the summary spans the whole period when the list is narrowed to one year', () => {
    render('/trends?year=2024', loaderData({ year: '2024' }));
    expect(container.querySelector('.ov-year-scope-note')?.textContent).toContain('2024');
  });

  it('shows no year-scope note without a year filter', () => {
    render('/trends', loaderData());
    expect(container.querySelector('.ov-year-scope-note')).toBeNull();
  });
});
