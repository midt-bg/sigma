...
**Проверено:** `pnpm --filter web typecheck` → 0; **122 теста** преминават; `pnpm audit --audit-level=high`
...
- **Втвърдяване:** AST guard (`node-sql-parser`) над структурния read-only guard (§9.4 — проследено като
  отделен hardening след ревюто на #80); HMAC-подпис на сървърните съобщения (§9.3); memoize
