...
export const DATA_TRAPS: string[] = [
  ...
  'Канонична база за всяка парична сума: `contracts.amount_eur IS NOT NULL`. НЕ филтрирай по ' +
    '`value_flag`: включи `ok`, `review`, `annex_suspect`, `annex_total_suspect`, `value_low` и поправените `value_suspect` редове.',
  ...
  '`value_flag` ∈ {ok, review, value_low, annex_suspect, annex_total_suspect, value_suspect} мени значението на стойността на реда, ' +
  ...
  'Идентификаторите са само за JOIN и за entity links — НИКОГА не ги показвай като видима колона в ' +
    ...
    '`eik:…` или `name:…`, `contracts.id` = `c:e:…`/`c:o:…` (композитен ключ, който ВГРАЖДА id-то на ' +
    'изпълнителя, напр. `c:e:00042-2025-0016:…:eik:175405647:1`) — сурови вътрешни ключове, безсмислени за ' +
    'читателя. За „кой" SELECT-вай ИМЕТО (`a.name` за възложител, `b.name` за изпълнител) като видима колона; ' +
    'за видим номер на договор ползвай УНП (`t.source_id`), НЕ `c.id`. id-то подавай само през механизма за ' +
    'връзки (`link.idCol`), не като `key`. Пример: `SELECT a.name, a.id AS authority_id, …` — показва се ' +
    '`name`, `authority_id` е само цел на връзката.',
