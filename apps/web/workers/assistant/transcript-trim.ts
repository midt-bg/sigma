...
function summaryContent(collapsed: readonly TranscriptMessage[]): string {
  const ordered = [...collapsed].sort(bySlot);
  const turnCount = new Set(ordered.map((m) => m.turnIndex)).size;
  ...
  const header = `[свита история: ${turnCount} по-стари хода]`;
 ...
 * Returns `[summary, ...keptVerbatim]` when anything collapses, else the messages unchanged.
...
export async function trimTranscript(
  ...
  const conversationId = collapsed[0]!.conversationId;

  const summary = await attachSignature(env, {
    role: SUMMARY_ROLE,
    content: summaryContent(collapsed),
    conversationId,
