...
// number. The rollups (sector_totals, authority_totals, …) are built with the invariant that every
// (count, sum) pair ranges over exactly the `amount_eur IS NOT NULL` rows; a live aggregate that
// disagrees means a filter or join drifted. On mismatch we BLOCK and surface the discrepancy — we
// never silently substitute one figure for the other, because either could be the wrong one.
// Money is REAL (float), so sums reconcile within a tolerance; counts must match exactly.
